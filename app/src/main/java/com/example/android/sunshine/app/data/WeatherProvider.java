...
public class WeatherProvider extends ContentProvider {
    ...
    static UriMatcher buildUriMatcher() {
        ...
        UriMatcher weatherUriMather = new UriMatcher(UriMatcher.NO_MATCH);

        // 2) Use the addURI function to match each of the types.  Use the constants from
        // WeatherContract to help define the types to the UriMatcher.
        weatherUriMather.addURI(CONTENT_AUTHORITY, PATH_WEATHER, WEATHER);
        weatherUriMather.addURI(CONTENT_AUTHORITY, PATH_LOCATION, LOCATION);
        weatherUriMather.addURI(CONTENT_AUTHORITY, WeatherEntry.
                buildWeatherLocation("*").getPath(), WEATHER_WITH_LOCATION);
        weatherUriMather.addURI(CONTENT_AUTHORITY, WeatherEntry. buildWeatherLocation("*").
                buildUpon().appendPath("#").build().getPath(), WEATHER_WITH_LOCATION_AND_DATE);

        // 3) Return the new matcher!
        return weatherUriMather;
    }
                        ...
                        String sortOrder)
    {
        ...
        //nmalov: note, if notifications on data change needed you should explicitly define that
...
//        //TODO:nmalov: Why is it here?
//        if (null == selection)
//            selection = "1";
        switch (sUriMatcher.match(uri)) {
            case WEATHER: {
                 result = db.delete(WeatherEntry.TABLE_NAME, selection, selectionArgs);
                break;
            }
            case LOCATION: {
                result = db.delete(LocationEntry.TABLE_NAME, selection, selectionArgs);
                break;
            }
            default:
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        switch(sUriMatcher.match(uri)) {
            case(WEATHER) : {
                result = db.update(
                        WeatherEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            }
            case (LOCATION) : {
                result = db.update(
                        LocationEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            }
            default:
